...
def check_sql_oracle_script(sitename, env, hostname, logpath, oracleuser, oraclepassword,
    ...
    try:
        ...
        message_conn_db = f'{sitestarttime}|{site}|{systemname}|{env}|ORACLE_DB|ERROR|ERROR: Failed to connect Oracle DB: {excep_conn_db}\r\n'
    ...
    if conn is not None:
        try:
            ...
            message_create_cursor = f'{sitestarttime}|{site}|{systemname}|{env}|ORACLE_DB|ERROR|ERROR: Failed Oracle DB - Create Cursor in Exception = {excep_create_cursor}\r\n'
    ...
    if sqlresult is not None:
        if oracleexpectedvalue and oracleevaluateoperator is not None:
            try:
                ...
                message_sql_eval = f'{sitestarttime}|{site}|{systemname}|{env}|ORACLE_DB|ERROR|Failed to evalute SQL Result: {excep_sql_eval}\r\n'
            ...
            message_compare_failed = f'{sitestarttime}|{site}|{systemname}|{env}|ORACLE_DB|ERROR|Result comparision failed due missing expected values or operator in config file'
        ...
        message_comp_failed_no_res = f'{sitestarttime}|{site}|{systemname}|{env}|ORACLE_DB|ERROR|SQL result comparision failed due to not obained SQL Result '
    ...
    if sqlcomparision == 'OK':
        message_validation_ok = f'{sitestarttime}|{site}|{systemname}|{env}|ORACLE_DB|OK|SQL condition validation OK - SQL scripts results are in defined conditon, records {sqlresult}\r\n'
    ...
    try:
        ...
        message_close_conn_failed = f'{sitestarttime}|{site}|{systemname}|{env}|ORACLE_DB|ERROR|Failed to close connection: {excep_db_close_conn}\r\n'
